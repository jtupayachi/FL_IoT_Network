#!/bin/bash
# Client 0 execution for MOON, FedALA, StatAvg, and DASHA methods - LSTM
    ...
    for var2 in $slr; do
        for mu in $MOON_mu; do
            for temp in $MOON_temperature; do
                ...
                python3 fl_testbed/version2/client/federated_client_RUL_MOON.py -cn 0 -cm 5 -e 1 -dfn 'M3_5_0_ddf_LSTM.pkl' -dfn_test_x '100_2_15_15_combined_offset_misalignment_M3.csv__client_centralizedtest_inputs.pkl' -dfn_test_y '100_2_15_15_combined_offset_misalignment_M3.csv__client_centralizedtest_out.pkl' -ip 172.18.0.2 2>&1 | tee LSTM_CLIENT0_MOON_${var}_slr_${var2}_mu_${mu}_temp_${temp}.txt
                echo "done"
            done
        done
    done
    ...
    for var2 in $slr; do
        for stat_w in $StatAvg_stat_weight; do
            for use_var in $StatAvg_use_variance; do
                ...
                python3 fl_testbed/version2/client/federated_client_RUL_StatAvg.py -cn 0 -cm 5 -e 1 -dfn 'M3_5_0_ddf_LSTM.pkl' -dfn_test_x '100_2_15_15_combined_offset_misalignment_M3.csv__client_centralizedtest_inputs.pkl' -dfn_test_y '100_2_15_15_combined_offset_misalignment_M3.csv__client_centralizedtest_out.pkl' -ip 172.18.0.2 2>&1 | tee LSTM_CLIENT0_StatAvg_${var}_slr_${var2}_statw_${stat_w}_usevar_${use_var}.txt
                echo "done"
            done
        done
    done
        ...
        for alpha_val in $DASHA_alpha; do
            for gamma_val in $DASHA_gamma; do
                for mom in $DASHA_momentum; do
                    ...
                    python3 fl_testbed/version2/client/federated_client_RUL_DASHA.py -cn 0 -cm 5 -e 1 -dfn 'M3_5_0_ddf_LSTM.pkl' -dfn_test_x '100_2_15_15_combined_offset_misalignment_M3.csv__client_centralizedtest_inputs.pkl' -dfn_test_y '100_2_15_15_combined_offset_misalignment_M3.csv__client_centralizedtest_out.pkl' -ip 172.18.0.2 2>&1 | tee LSTM_CLIENT0_DASHA_${var}_slr_${var2}_alpha_${alpha_val}_gamma_${gamma_val}_mom_${mom}.txt
                    echo "done"
                done
            done
        done
