#!/bin/bash
# Client 2 execution for MOON, FedALA, StatAvg, and DASHA methods - MLP
    ...
    for var2 in $slr; do
        for mu in $MOON_mu; do
            for temp in $MOON_temperature; do
                ...
                python3 fl_testbed/version2/client/federated_client_OFFSET_MOON.py -cn 2 -cm 5 -e 1 -ip 172.18.0.8 -dfn_test_x '100_1_15_15_combined_offset_misalignment_M3.csv__client_centralizedX_test.pkl' -dfn_test_y '100_1_15_15_combined_offset_misalignment_M3.csv__client_centralizedy_test.pkl' -dfn 'M3_5_2_ddf_MLP.pkl' 2>&1 | tee MLP_CLIENT2_MOON_${var}_slr_${var2}_mu_${mu}_temp_${temp}.txt
                echo "done"
            done
        done
    done
    ...
    for var2 in $slr; do
        for eta in $FedALA_eta; do
            for threshold in $FedALA_threshold; do
                ...
                python3 fl_testbed/version2/client/federated_client_OFFSET_FedALA.py -cn 2 -cm 5 -e 1 -ip 172.18.0.8 -dfn_test_x '100_1_15_15_combined_offset_misalignment_M3.csv__client_centralizedX_test.pkl' -dfn_test_y '100_1_15_15_combined_offset_misalignment_M3.csv__client_centralizedy_test.pkl' -dfn 'M3_5_2_ddf_MLP.pkl' 2>&1 | tee MLP_CLIENT2_FedALA_${var}_slr_${var2}_eta_${eta}_threshold_${threshold}.txt
                echo "done"
            done
        done
    done
    ...
    for var2 in $slr; do
        for stat_w in $StatAvg_stat_weight; do
            for use_var in $StatAvg_use_variance; do
                ...
                python3 fl_testbed/version2/client/federated_client_OFFSET_StatAvg.py -cn 2 -cm 5 -e 1 -ip 172.18.0.8 -dfn_test_x '100_1_15_15_combined_offset_misalignment_M3.csv__client_centralizedX_test.pkl' -dfn_test_y '100_1_15_15_combined_offset_misalignment_M3.csv__client_centralizedy_test.pkl' -dfn 'M3_5_2_ddf_MLP.pkl' 2>&1 | tee MLP_CLIENT2_StatAvg_${var}_slr_${var2}_statw_${stat_w}_usevar_${use_var}.txt
                echo "done"
            done
        done
    done
        ...
        for alpha_val in $DASHA_alpha; do
            for gamma_val in $DASHA_gamma; do
                for mom in $DASHA_momentum; do
                    ...
                    python3 fl_testbed/version2/client/federated_client_OFFSET_DASHA.py -cn 2 -cm 5 -e 1 -ip 172.18.0.8 -dfn_test_x '100_1_15_15_combined_offset_misalignment_M3.csv__client_centralizedX_test.pkl' -dfn_test_y '100_1_15_15_combined_offset_misalignment_M3.csv__client_centralizedy_test.pkl' -dfn 'M3_5_2_ddf_MLP.pkl' 2>&1 | tee MLP_CLIENT2_DASHA_${var}_slr_${var2}_alpha_${alpha_val}_gamma_${gamma_val}_mom_${mom}.txt
                    echo "done"
                done
            done
        done
