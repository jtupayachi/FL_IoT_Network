#!/bin/bash
# New FL Methods: MOON, FedALA, StatAvg, DASHA for LSTM (RUL)
...
for var in $alphas; do
    ...
    for client in 0 1 2 3 4; do
        python3 fl_testbed/version2/client/independent.py  -ml 2  -cn 15 -cm 15 -e 100 -dfn   "M3_5_${client}_ddf_LSTM.pkl" -dfn_test_x   '100_2_15_15_combined_offset_misalignment_M3.csv__client_centralizedtest_inputs.pkl' -dfn_test_y   '100_2_15_15_combined_offset_misalignment_M3.csv__client_centralizedtest_out.pkl'   -ip 172.18.0.2 2>&1 | tee out_server_14_M3_5_${client}_ddf_LSTM_idp_${var}.txt2
    done
    ...
    for var2 in $slr; do
        for mu in $MOON_mu; do
            for temp in $MOON_temperature; do
                ...
                python3 fl_testbed/version2/server/federated_server_RUL_MOON.py -mu ${mu} -temperature ${temp} -slr ${var2} -cm 5 -e 1 --rounds 1000 -ip 172.18.0.2 -dfn_test_x '100_2_15_15_combined_offset_misalignment_M3.csv__client_centralizedtest_inputs.pkl' -dfn_test_y '100_2_15_15_combined_offset_misalignment_M3.csv__client_centralizedtest_out.pkl' -dfn 'M3_5_0_ddf_LSTM.pkl' 2>&1 | tee LSTM_TESLA_MOON_${var}_slr_${var2}_mu_${mu}_temp_${temp}.txt
                echo "MOON done"
            done
        done
    done
    ...
    for var2 in $slr; do
        for eta in $FedALA_eta; do
            for threshold in $FedALA_threshold; do
                ...
                python3 fl_testbed/version2/server/federated_server_RUL_FedALA.py -eta ${eta} -threshold ${threshold} -slr ${var2} -cm 5 -e 1 --rounds 1000 -ip 172.18.0.2 -dfn_test_x '100_2_15_15_combined_offset_misalignment_M3.csv__client_centralizedtest_inputs.pkl' -dfn_test_y '100_2_15_15_combined_offset_misalignment_M3.csv__client_centralizedtest_out.pkl' -dfn 'M3_5_0_ddf_LSTM.pkl' 2>&1 | tee LSTM_TESLA_FedALA_${var}_slr_${var2}_eta_${eta}_threshold_${threshold}.txt
                echo "FedALA done"
            done
        done
    done
    ...
    for var2 in $slr; do
        for stat_w in $StatAvg_stat_weight; do
            for use_var in $StatAvg_use_variance; do
                ...
                python3 fl_testbed/version2/server/federated_server_RUL_StatAvg.py -stat_weight ${stat_w} -use_variance ${use_var} -slr ${var2} -cm 5 -e 1 --rounds 1000 -ip 172.18.0.2 -dfn_test_x '100_2_15_15_combined_offset_misalignment_M3.csv__client_centralizedtest_inputs.pkl' -dfn_test_y '100_2_15_15_combined_offset_misalignment_M3.csv__client_centralizedtest_out.pkl' -dfn 'M3_5_0_ddf_LSTM.pkl' 2>&1 | tee LSTM_TESLA_StatAvg_${var}_slr_${var2}_statw_${stat_w}_usevar_${use_var}.txt
                echo "StatAvg done"
            done
        done
    done
        ...
        for alpha_val in $DASHA_alpha; do
            for gamma_val in $DASHA_gamma; do
                for mom in $DASHA_momentum; do
                    ...
                    python3 fl_testbed/version2/server/federated_server_RUL_DASHA.py -alpha ${alpha_val} -gamma ${gamma_val} -momentum ${mom} -slr ${var2} -cm 5 -e 1 --rounds 1000 -ip 172.18.0.2 -dfn_test_x '100_2_15_15_combined_offset_misalignment_M3.csv__client_centralizedtest_inputs.pkl' -dfn_test_y '100_2_15_15_combined_offset_misalignment_M3.csv__client_centralizedtest_out.pkl' -dfn 'M3_5_0_ddf_LSTM.pkl' 2>&1 | tee LSTM_TESLA_DASHA_${var}_slr_${var2}_alpha_${alpha_val}_gamma_${gamma_val}_mom_${mom}.txt
                    echo "DASHA done"
                done
            done
        done
