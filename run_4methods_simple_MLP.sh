...
sudo docker exec fl_moon_server bash -c "cd /workspace && rm -f MLP_*.txt *.log"
sudo docker exec fl_fedala_server bash -c "cd /workspace && rm -f MLP_*.txt *.log"
sudo docker exec fl_statavg_server bash -c "cd /workspace && rm -f MLP_*.txt *.log"
sudo docker exec fl_dasha_server bash -c "cd /workspace && rm -f MLP_*.txt *.log"
...
run_moon_experiment() {
    ...
    sudo docker exec fl_moon_server pkill -9 -f "federated_server_OFFSET_MOON" || true
    for cid in 0 1 2 3 4; do
        sudo docker exec fl_moon_client${cid} pkill -9 -f "federated_client_OFFSET_MOON" || true
    done
    sleep 2
    ...
    sudo docker exec -d fl_moon_server bash -c "cd /workspace && \
        ...
        > MLP_MOON_alpha_${alpha}_tau_${tau_val}.txt 2>&1"
    ...
    for client_id in 0 1 2 3 4; do
        sudo docker exec -d fl_moon_client${client_id} bash -c "cd /workspace && \
            python3 fl_testbed/version2/client/federated_client_OFFSET_MOON.py \
            -dfn 'M3_5_${client_id}_ddf_MLP.pkl' \
            -cm 5 -e 1 -ip 172.18.1.10:$port -cn ${client_id} \
            -dfn_test_x '100_1_15_15_combined_offset_misalignment_M3.csv__client_centralizedX_test.pkl' \
            -dfn_test_y '100_1_15_15_combined_offset_misalignment_M3.csv__client_centralizedy_test.pkl' \
            > MLP_MOON_client${client_id}_alpha_${alpha}_tau_${tau_val}.log 2>&1"
    done
    
    # Wait for FL to complete (monitor server process)
    while sudo docker exec fl_moon_server pgrep -f "federated_server_OFFSET_MOON" > /dev/null 2>&1; do
        sleep 10
    done
...
run_fedala_experiment() {
    ...
    # Kill any lingering processes
    sudo docker exec fl_fedala_server pkill -9 -f "federated_server_OFFSET_FedALA" || true
    for cid in 0 1 2 3 4; do
        sudo docker exec fl_fedala_client${cid} pkill -9 -f "federated_client_OFFSET_FedALA" || true
    done
    sleep 2
    ...
    sudo docker exec -d fl_fedala_server bash -c "cd /workspace && \
        ...
        -dfn_test_y '100_1_15_15_combined_offset_misalignment_M3.csv__client_centralizedy_test.pkl' \
        -dfn 'M3_5_0_ddf_MLP.pkl' \
        > MLP_FedALA_alpha_${alpha}_tau_${tau_val}.txt 2>&1"
    
    sleep 10
    
    for client_id in 0 1 2 3 4; do
        sudo docker exec -d fl_fedala_client${client_id} bash -c "cd /workspace && \
            python3 fl_testbed/version2/client/federated_client_OFFSET_FedALA.py \
            -dfn 'M3_5_${client_id}_ddf_MLP.pkl' \
            -cm 5 -e 1 -ip 172.18.2.10:$port -cn ${client_id} \
            -dfn_test_x '100_1_15_15_combined_offset_misalignment_M3.csv__client_centralizedX_test.pkl' \
            -dfn_test_y '100_1_15_15_combined_offset_misalignment_M3.csv__client_centralizedy_test.pkl' \
            > MLP_FedALA_client${client_id}_alpha_${alpha}_tau_${tau_val}.log 2>&1"
    done
    
    while sudo docker exec fl_fedala_server pgrep -f "federated_server_OFFSET_FedALA" > /dev/null 2>&1; do
        sleep 10
    done
...
run_statavg_experiment() {
    ...
    # Kill any lingering processes
    sudo docker exec fl_statavg_server pkill -9 -f "federated_server_OFFSET_StatAvg" || true
    for cid in 0 1 2 3 4; do
        sudo docker exec fl_statavg_client${cid} pkill -9 -f "federated_client_OFFSET_StatAvg" || true
    done
    sleep 2
    ...
    sudo docker exec -d fl_statavg_server bash -c "cd /workspace && \
        ...
        -dfn_test_y '100_1_15_15_combined_offset_misalignment_M3.csv__client_centralizedy_test.pkl' \
        -dfn 'M3_5_0_ddf_MLP.pkl' \
        > MLP_StatAvg_alpha_${alpha}_slr_${slr_val}.txt 2>&1"
    
    sleep 10
    
    for client_id in 0 1 2 3 4; do
        sudo docker exec -d fl_statavg_client${client_id} bash -c "cd /workspace && \
            python3 fl_testbed/version2/client/federated_client_OFFSET_StatAvg.py \
            -dfn 'M3_5_${client_id}_ddf_MLP.pkl' \
            -cm 5 -e 1 -ip 172.18.3.10:$port -cn ${client_id} \
            -dfn_test_x '100_1_15_15_combined_offset_misalignment_M3.csv__client_centralizedX_test.pkl' \
            -dfn_test_y '100_1_15_15_combined_offset_misalignment_M3.csv__client_centralizedy_test.pkl' \
            > MLP_StatAvg_client${client_id}_alpha_${alpha}_slr_${slr_val}.log 2>&1"
    done
    
    while sudo docker exec fl_statavg_server pgrep -f "federated_server_OFFSET_StatAvg" > /dev/null 2>&1; do
        sleep 10
    done
...
run_dasha_experiment() {
    ...
    # Kill any lingering processes
    sudo docker exec fl_dasha_server pkill -9 -f "federated_server_OFFSET_DASHA" || true
    for cid in 0 1 2 3 4; do
        sudo docker exec fl_dasha_client${cid} pkill -9 -f "federated_client_OFFSET_DASHA" || true
    done
    sleep 2
    ...
    sudo docker exec -d fl_dasha_server bash -c "cd /workspace && \
        ...
        -dfn_test_y '100_1_15_15_combined_offset_misalignment_M3.csv__client_centralizedy_test.pkl' \
        -dfn 'M3_5_0_ddf_MLP.pkl' \
        > MLP_DASHA_alpha_${alpha}_slr_${slr_val}.txt 2>&1"
    
    sleep 10
    
    for client_id in 0 1 2 3 4; do
        sudo docker exec -d fl_dasha_client${client_id} bash -c "cd /workspace && \
            python3 fl_testbed/version2/client/federated_client_OFFSET_DASHA.py \
            -dfn 'M3_5_${client_id}_ddf_MLP.pkl' \
            -cm 5 -e 1 -ip 172.18.4.10:$port -cn ${client_id} \
            -dfn_test_x '100_1_15_15_combined_offset_misalignment_M3.csv__client_centralizedX_test.pkl' \
            -dfn_test_y '100_1_15_15_combined_offset_misalignment_M3.csv__client_centralizedy_test.pkl' \
            > MLP_DASHA_client${client_id}_alpha_${alpha}_slr_${slr_val}.log 2>&1"
    done
    
    while sudo docker exec fl_dasha_server pgrep -f "federated_server_OFFSET_DASHA" > /dev/null 2>&1; do
        sleep 10
    done
...
if echo "$METHODS_TO_RUN" | grep -q "\<MOON\>"; then
    sudo docker cp fl_moon_server:/workspace/MLP_MOON_*.txt ./ 2>/dev/null || echo "No MOON results found"
fi
if echo "$METHODS_TO_RUN" | grep -q "\<FedALA\>"; then
    sudo docker cp fl_fedala_server:/workspace/MLP_FedALA_*.txt ./ 2>/dev/null || echo "No FedALA results found"
fi
if echo "$METHODS_TO_RUN" | grep -q "\<StatAvg\>"; then
    sudo docker cp fl_statavg_server:/workspace/MLP_StatAvg_*.txt ./ 2>/dev/null || echo "No StatAvg results found"
fi
if echo "$METHODS_TO_RUN" | grep -q "\<DASHA\>"; then
    sudo docker cp fl_dasha_server:/workspace/MLP_DASHA_*.txt ./ 2>/dev/null || echo "No DASHA results found"
fi
