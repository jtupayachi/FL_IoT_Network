...
#         """Train the model for one round with proper validation - OPTIMIZED VERSION"""
#         self.current_round = config.get("current_round", 0)
        
#         # OPTIMIZATION: Only run CV in first round or periodically (not every round)
#         # This is the BIGGEST performance gain - CV runs 5x less frequently
#         run_cv = (
#             self.current_round == 0 or                    # First round
#             self.current_round % 5 == 0 or               # Every 5 rounds
#             self.current_round == self.total_rounds - 1  # Final round
#         )
        
#         if run_cv:
#             # OPTIMIZATION: Use fewer folds during training (faster but still useful)
#             k_folds = min(3, self.hyperparams.k_folds)  # Max 3 folds instead of 5
...
#         else:
#             # OPTIMIZATION: Skip CV for most rounds to save massive computation time
#             cv_metrics = {
#                 "fold": [0],
#                 "train_loss": [0], 
#                 "val_loss": [0],
...
#         # Now train on full training dataset for federated learning
#         self.set_parameters(parameters)
        
#         epochs = config.get("local_epochs", self.hyperparams.local_epochs)
#         batch_size = config.get("batch_size", self.hyperparams.batch_size)
        
#         # Training loop on full training dataset
#         self.model.train()
#         train_losses = []
...
#         # OPTIMIZATION: Reduce logging frequency to save I/O time
...
#         else:
#             # Minimal logging for intermediate rounds
#             print(f"Round {self.current_round}: train_loss={train_loss:.4f}, val_loss={val_loss:.4f}, val_r2={val_metrics['r2']:.4f}")
        
#         # Prepare results - ONLY include simple types that Flower can handle
#         results = {
#             "loss": float(val_loss),  # Report validation loss to server
...
#         # OPTIMIZATION: Reduce CSV logging frequency to save I/O time
